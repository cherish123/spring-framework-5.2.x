...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 调用bean工厂的后置处理器.1.会在此将class扫描成beanDefinition；2.会调用bean工厂后置处理器完成创世纪的类的实例化
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 忽略以下接口bean的 接口函数方法。再populateBean时 因为以下接口都有setXXX方法，
		 * 这些方法不特殊处理将会自动注入容器中的bean
		 */
		...
		//注册了一个事件监听器探测器后置处理器接口，解析接口方式监听器
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//获取两处存储BeanFactoryPostProcessor的对象 传入供接下来的使用,getBeanFactoryPostProcessors()默认是空的，只有当我们通过context.addBeanFactoryPostProcessor()才会加入进来
		//1.当前bean工厂；2.我们自己调用addBeanFactoryPostProcessor的自定义BeanFactoryPostProcessor
	...
	protected void registerListeners() {
		...
		//获取bean定义中的监听器对象 接口方式的监听器就是再这里注册到多播器里面的
